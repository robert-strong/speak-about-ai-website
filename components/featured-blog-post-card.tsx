    ...
    <article className="group relative flex flex-col md:flex-row items-center gap-8 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 bg-white overflow-hidden">
      {featuredImageUrl && (
        <div className="md:w-1/2 w-full h-64 md:h-80 relative shrink-0 overflow-hidden rounded-md">
          <Link href={`/resources/${post.slug}`} className="block w-full h-full">
            <Image
      ...
      <div className="flex flex-col justify-center">
        {post.categories && post.categories.length > 0 && (
          <Link
            href={`/resources?category=${post.categories[0].slug}`} // Link to the first category
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3 leading-tight">
          <Link href={`/resources/${post.slug}`} className="hover:text-blue-700 transition-colors">
            {post.title}
          </Link>
        </h2>
        ...
        <Link href={`/resources/${post.slug}`} className="text-blue-600 hover:underline font-semibold self-start text-base">
          Read article &rarr;
        </Link>
      </div>
    </article>
